...
static int comm_link_packet_ready(comm *comm) {
...
int comm_link_rx(comm *comm, unsigned char c, unsigned char *fin) {
  ...
  case COMM_LNK_STATE_LEN:
	if (c > COMM_LNK_MAX_DATA-1) {
      comm->lnk.state = comm->conf & COMM_CONF_SKIP_PREAMPLE ? COMM_LNK_STATE_LEN : COMM_LNK_STATE_PRE;
  ...
  case COMM_LNK_STATE_DAT:
    ...
    if (comm->lnk.ix == comm->lnk.len) {
      if (comm->conf & COMM_CONF_SKIP_CRC) {
        comm->lnk.ix = 0;
        comm->lnk.state = comm->conf & COMM_CONF_SKIP_PREAMPLE ? COMM_LNK_STATE_LEN : COMM_LNK_STATE_PRE;
        int res = comm_link_packet_ready(comm);
        if (fin) *fin = 1;
        return res;
      } else {
        comm->lnk.ix = 1;
